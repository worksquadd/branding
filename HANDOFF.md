...
Дальше: материалы складывать в `assets/`; `serve_assets.py` покажет их автоматически. Правила скрытия находятся в `assets/.ignore` (сейчас `.*`, `.` и `..`). В listing отображаются только названия; файлы скачиваются по клику, папки открываются. Workflow использует repo-level secrets `WORKSQUAD_HOST`, `WORKSQUAD_USER` и `WORKSQUAD_SSH_PRIVATE_KEY`, соответствующие SSH host `worksquad` (`maratik@2.26.120.50`), и запускает Docker через разрешённый sudo. Он ждёт готовности контейнера до 10 секунд перед подключением Nginx. Устаревшие org secret/variable RIFTY_1 удалены; временный локальный deploy-ключ удалён после загрузки в repo secret.
